...
const getPostsById = (id) => connection.query('SELECT posts.id,posts.post_text,users.user_name,posts.posted_at_time FROM posts left join users on posts.users_id = users.id where posts.id = $1', [id]);
